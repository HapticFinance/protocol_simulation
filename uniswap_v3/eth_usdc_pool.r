...
initPool <- function () {
  ...
  for (r in 1:nrow(pool_state))   {
    ...
    if (r == 1) {
      ...
      if (randomPrice > previousPrice) {
        ...
        deltaTokens = pool_state[r-1, 1] - getx(L, randomPrice, maxP) #deltaTokens + xNew
        ...
        deltaTokens = pool_state[r-1, 1] - gety(L, randomPrice, Pa) #deltaTokens + yNew
      ...
      if ((pool_state[r, 2] -  pool_state[r-1, 2]) > 0) {
        volumeUSDC = pool_state[r, 2] -  pool_state[r-1, 2]
      } else {
        volumeUSDC = -1 * (pool_state[r, 2] - pool_state[r-1, 2]) 
      }
  ...
  # Pool balances
  deltaUSDC = (pool_state[r, 1]) - x
  if (x > (pool_state[r, 1] )) {
    deeltaUSDC = -1 * (x - pool_state[r, 1] )
  }
