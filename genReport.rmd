...
```{r echo=FALSE, warning=FALSE, results='asis'}
  ...
  dfStakersInfo %>%
    kbl( caption = paste("Stakers info"), booktabs = T) %>%
      kable_styling(latex_options = "HOLD_position") %>%
        kable_classic(full_width = T, html_font = "Cambria") %>%
            print()
...
Total liquidations  `r total_liquidations`, total stakers liquidated `r length(unique(stakers_liquidated))`
...
emissions_weekly = ts(data = weekly_emissions, start = 1, end = nrow(historicalPricesHAP[,]), frequency = 1,  deltat = 1)
matplot(emissions_weekly, type = "l", main = "Emissions", xlab = "Weeks", ylab = "HAP", col=c("blue"), bg=c("grey"))
