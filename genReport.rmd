...
longTable <- ifelse(n_borrowers > 30 || n_stakers > 30, TRUE, FALSE)
```
...
color.me <- which(dfClegend$Color == "brown")
...
```{r echo=FALSE, warning=FALSE}
#print(glue::glue("Loaded {nrow(historicalPricesETH)} weeks for ETH and {nrow(historicalPricesHAP)} for HAP"))
#cat("abc", sep="\n\n") 
```
...
for (k in 1:nrow(historicalPricesETH)) {
  ...
  #borrowers <- borrowers_cp

  #table <- kbl(borrowers, col.names=labels_borrowers_2, caption = paste("Week", k, "ETH/USD", 0,  sep=" "), full_width=TRUE)
  #tables <- c(tables, kable_classic(table, full_width = F, html_font = "Cambria"))
  #separators <- c(separators, "\n\n<!-- -->\n\n")
   

  dfB <- data.frame(borrowers_cp[,])
  colnames(dfB) <- labels_borrowers_2
  dfB <-  setNames(dfB, labels_borrowers_2)
  dfB <- dfB[order(-dfB$sUSD),]

  color.me <- which(dfB$LiqWeek > 0 & dfB$I/L == 0)
  rows <- which(dfB$I/L > 0 & dfB$ETH == 0 )
  ...
  dfB %>%
    ...
    #mutate(LiqWeek = cell_spec(LiqWeek, background=ifelse(LiqWeek > 0, "red", "white"))) %>% 
    kbl( caption = paste("Week", k, "- ETH/USD", prettyETH,  sep=" "), booktabs = T, escape=F, longtable=longTable) %>%
      ...
      kable_styling(latex_options = "HOLD_position") %>%
        kable_classic(full_width = F, html_font = "Cambria") %>%
            #row_spec(color.me, bold = F, color = "white", background="red" ) %>%
              #row_spec(rows, bold = F, color = "white", background="brown") %>%
  ...
  #cat("\\section{Part:", k, "}")
  #cat("\\begin{verbatim}")
  #print(kable_classic(table, full_width = F, html_font = "Cambria"))
  #cat("\\end{verbatim}")
}
...
callback <- function(data) {
  cat("# Stakers {.unlisted .unnumbered}", "\n")
  return(data)
}
...
for (k in 1:nrow(historicalPricesETH)) {
    ...
    dfStakers <- data.frame(stakers_cp[,])
    colnames(dfStakers) <- labels_stakers_short
    dfStakers <-  setNames(dfStakers, labels_stakers_short)
    #dfStakers <- dfStakers[order(-dfStakers$TDA),]

    color.me <- which(dfStakers$LiqWeek != 0)
    rows <- which(dfStakers$FixCratio == 1)
    ...
    dfStakers %>%
      ...
      #mutate(across(!name, as.factor))
      #mutate(LiqWeek = cell_spec(LiqWeek, background=ifelse(LiqWeek > 0 , "red", "white"))) %>% 
      kbl( caption = paste("Week", k, "- HAP/USD", prettyHap,  sep=" "), booktabs = T, escape=T, longtable=longTable) %>%
    ...
    #dfStakers %>%
    #reduce(1:ncol(dfStakers), function(x, y) {
    #  col <- dfStakers[, y]
    #  column_spec(x, y, background = case_when(
    #    labels_stakers_short[y] == "LiqWeek" & col == 1 ~ "red",
    #    labels_stakers_short[y] == "Comp-Week" & col != 0 ~ "brown",
    #    TRUE ~ "white"))
    #}, .init = .) %>%
    #kbl(col.names=labels_stakers_short, caption = paste("Week", k, "- HAP/USD", prettyHap,  sep=" "), booktabs = T) %>%
    #    kable_styling(latex_options = "HOLD_position") %>%
    #      kable_classic(full_width = F, html_font = "Cambria") %>%
            #row_spec(color.me, bold = F, color = "white", background = "red" ) %>%
                #row_spec(rows, bold = F, color = "white", background = "green" ) %>%
                  #column_spec(1:6, background="white", color="black") %>%
    #              print()
}
```
