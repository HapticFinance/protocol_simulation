...
borrowers_final_ = get(glue::glue("borrowers_week_{nrow(historicalPricesETH)}"))
...
Impermanent loss compensations are disbursed denominated in TDA.
...
for (k in 1:nrow(historicalPricesETH)) {
  ...
  stakers_cp <- stakers_cp[, c(1, 2, 9, 3, 4, 5, 6, 7)]
...
total_tda_burned <- prettyNum(totalBurnedTDA, scientific=F, big.mark=",")
...
* Total burned **`r total_tda_burned`** TDA
...
for (j in 1:nrow(historicalPricesETH)) {
  ...
  for (x in 1:nrow(stakers))   {
    ...
    APR = (totalRewards * historicalPricesHAP[j, 3]) * 52 / totalStaked * historicalPricesHAP[j, 3]
...
ts(data = avgAprs, start = 1, end = nrow(historicalPricesHAP[,]), frequency = 1,  deltat = 1) %>%
matplot( type = "l", main = "Avg. APR", xlab = "Weeks", ylab = "%", col=c("blue"), bg=c("grey"))

total_hap_sold <- prettyNum(totalHapRequired, scientific=F, big.mark=",")
...
#ts(data = hap_required_period, start = 1, end = nrow(historicalPricesETH), frequency = 1,  deltat = 1) %>%
...
* **`r prettyNum((30000000 - totalHapRequired) + (total_emissions * 0.3), scientific=F, big.mark=",")`** HAP
...
```{r echo=FALSE, results='asis', warning=FALSE}
  ...
  ts(data = treasury_balances_period, start = 1, end = nrow(historicalPricesETH), frequency = 1,  deltat = 1) %>%
...
* Market cap **`r prettyNum((100000000 + total_emissions) * historicalPricesHAP[nrow(historicalPricesHAP), 3], scientific=F, big.mark=",")`** USD

# Tokenized Debt Asset (TDA)

## Dex performance

TDA price is `r prettyNum(poolState[4], scientific=F, big.mark=",")` USD  
Total sold is **`r prettyNum(poolState[6], scientific=F, big.mark=",")`** TDA - total sales `r poolState[5]`  
Total purchased is **`r prettyNum(poolState[8], scientific=F, big.mark=",")`** TDA  - total purchases `r poolState[7]`  
